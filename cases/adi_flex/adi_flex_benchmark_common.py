...
# --- Poisson helpers, kept local: the repository is deliberately scipy-free ---
# (`mab` and `fnv` hand-roll the same math). Semantics match scipy.stats.poisson
# where the benchmarks used it: `ppf(q)` is the smallest k with cdf(k) >= q.
def poisson_pmf(k, rate: float) -> np.ndarray:
    """pmf at integer(s) k for a Poisson(rate); rate 0 is the point mass at 0."""
    k = np.atleast_1d(np.asarray(k, dtype=float))
    if rate <= 0.0:
        return np.where(k == 0, 1.0, 0.0)
    lg = np.array([math.lgamma(x + 1.0) for x in k])
    return np.exp(k * math.log(rate) - rate - lg)
...
def poisson_cdf(k: int, rate: float) -> float:
    ...
    if k < 0:
        return 0.0
    return float(poisson_pmf(np.arange(int(k) + 1), rate).sum())


def poisson_sf(k: int, rate: float) -> float:
    """P(X > k)."""
    return 1.0 - poisson_cdf(k, rate)


def poisson_ppf(q: float, rate: float) -> int:
    """smallest k with P(X <= k) >= q (scipy's discrete ppf convention)."""
    if rate <= 0.0:
        return 0
    k, c = 0, float(poisson_pmf(0, rate)[0])
    while c < q:
        k += 1
        c += float(poisson_pmf(k, rate)[0])
    return k
...
def _truncated_poisson(rate: float) -> np.ndarray:
    ...
    if rate == 0:
        return np.array([1.0])
    d_max = poisson_ppf(1.0 - _TAIL, rate) + 1
    pmf = poisson_pmf(np.arange(d_max + 1), rate)
    return pmf / pmf.sum()
