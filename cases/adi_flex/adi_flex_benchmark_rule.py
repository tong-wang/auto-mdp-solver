...
from adi_flex_benchmark_common import U_MAX, U_MIN, ObsView, _truncated_poisson, EVAL_HEADER, eval_row
...
def protection_level(scenario: AdiFlexScenario, policy: str) -> int:
    ...
    if policy == "plmax":
        ...
        s = int(stats.poisson.ppf(1.0 - 0.001, scenario.lambda0)) if scenario.lambda0 > 0 else 0
        while stats.poisson.sf(s, scenario.lambda0) >= 0.001:
            s += 1
        return s
...
def protection_ladder(scenario: AdiFlexScenario, policy: str) -> tuple[int, ...]:
    ...
    for t in range(1, scenario.T_dl):
        ...
        if rate <= 0.0:
            out.append(0)
            continue
        cum = int(stats.poisson.ppf(q, rate))
        while stats.poisson.cdf(cum, rate) < q:    # ppf can land a step low
            cum += 1
