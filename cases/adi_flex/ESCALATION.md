...
scope:     operator — *"look at the proposal there and apply it here at
           ...
           `rl_test/game2048/UPSTREAM_PROPOSAL_config_registry.md` as a
...
quarantine split the campaign's code across `rl_test-adi_flex` and
`rl_test-adi_reserve`, and an audit rooted at its own tree reported green over
...
initial:   a second worktree, `rl_test-adi_reserve` on branch
           `adi_flex_reserve`, created for F31: a `mdp_tuning` trial re-execs
...
isolated worktree (`rl_test-adi_reserve`, branch `adi_flex_reserve`) precisely
...
protocol. Built and gated in the isolated worktree `rl_test-adi_reserve`
(branch `adi_flex_reserve`) so nothing could reach A2b's live study (F31).
