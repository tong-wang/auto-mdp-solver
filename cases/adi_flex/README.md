...
| policy | role | mean cost | SE | vs AP bound | vs the bar |
|---|---|---|---|---|---|
| AP relaxation | `relaxed` | **332.8232** | — | — (lower bound) | 95.9% |
...
| PPO — tuned `order_protection`/`vec`, categorical head (`sc4/g6/a0/h4`), *the crown it replaced* — one artifact | `feasible` | 337.4015 | — | +1.38% | 97.2% |
| PPO — the **derived rung** (spec §8.6 defaults, no tuning) | `feasible` | 345.9032 | 0.56 | +3.93% | 99.6% |
| PL(σ) + terminal σ=0 (off-bar variant) | `feasible` | 345.9955 | — | +3.96% | 99.7% |
| **PL(σ), ladder (4, 1) — the bar** | `feasible` | **347.1749** | — | +4.31% | — |
| PL(Σ) | `feasible` | 350.8833 | — | +5.43% | 101.1% |
| PL(0) | `feasible` | 364.7830 | — | +9.60% | 105.1% |

There is no `myopic` row: that heuristic is derived for `L = 0` and refuses to
run on this board.

Scope: the ★ and reference rows are **6-seed means** at the protocol (seeds
