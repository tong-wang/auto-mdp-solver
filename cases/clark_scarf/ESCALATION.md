...
| **A3** — remaining Stage-6 packaging | `PLAYBOOK.md` is written; what is left is whatever a contribution pass needs |
| **re-gate at v0.8.10** | gates last run at v0.8.3; the pin has since gained the `benchmarks` block, `run.provenance` and the ordering band |
| **adopt `benchmarks` in the IR** | the block now exists upstream (#23/#26); this domain still carries `role` in a README column, the interim D9 home |
