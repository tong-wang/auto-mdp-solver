...
| `inv_single` — single-item periodic-review inventory control | Textbook periodic-review inventory: Herbert Scarf's `(s, S)` setting ("The Optimality of (s, S) Policies in the Dynamic Inventory Problem", 1960), with the finite-horizon, lead-time, stochastic-lead-time and lost-sales variants as separate instances. | A **generalist over a parameter grid, tested on a held-out finer grid**: trained on one grid's sampler, scored by *enumerating* every cell, then re-scored on a 54-cell grid whose held-out cells are stratified one axis at a time — `mab`'s LV10 is one policy over a *one-axis* grid and `fnv` enumerates a design grid without training a generalist over it. The protocol buys the case's headline result: a policy interpolates a parameter it **reads** (held-out cost regimes score 96.5% of each cell's exact DP, above the trained cells' 95.8%) and does **not** interpolate one that changes **which state components are occupied** (an unseen lead time collapses to 55.1%) — invisible in advance, because padding keeps the observation the same width. Also the only case with an exact optimum on both sides of the fixed-cost line **and** the lost-sales line: the `(u, p1)` lost-sales DP is built here, and against it textbook base-stock is 8.66% above optimal. First case to use §14.0's `probe_required: false` to argue a readback exception in the IR rather than in prose. | ≤ v0.10.2 (#E1–#E18); v0.10.4 (#E19–#E24) | v0.10.9 |
