...
|---|---|---|---|---|
| `secretary` — classical no-information secretary problem | Dennis V. Lindley, “Dynamic Programming and Decision Theory,” *Applied Statistics* **10**(1), 1961; John P. Gilbert and Frederick Mosteller, “Recognizing the Maximum of a Sequence,” *JASA* **61**(313), 1966. | A **one-shot optimal-stopping decision with reset-time latent randomness**: the scenario contains only `N=100`, while each episode draws its own candidate permutation into state. Exact backward induction and the independently constructed finite-`N` threshold agree at skip 37; PPO reaches the exact protocol bar and its §14 readback recovers that rule on every valid observation state. | v0.11.4 | v0.11.4 |
| `clark_scarf` — serial multi-echelon inventory | Andrew J. Clark and Herbert Scarf, "Optimal Policies for a Multi-Echelon Inventory Problem", *Management Science* **6**(4), 1960 — read from the 2004 reprint, *Management Science* **50**(12S), 1782–1790. Model is §2–§3, generalized to N levels. | A **coupled multi-stage decision**: N simultaneous shipments per period, each clipped to what its source holds, so the top of the chain anticipates demand it never sees. Carries an `exact` DP verified against a brute-force joint-state solve rather than taken on the theorems' word — the first draft was 30% suboptimal and entirely convincing. What the second campaign stresses that the pipeline had not met: a **tuned three-way comparison of action heads whose ordering fully inverts its untuned ranking** (four heads ranked at L1, the three viable ones tuned) (the head 39 units behind at L1 wins once each is tuned at its own centre), a §14 `confirm` stance answered by **enumerating the policy as a function** because the availability clip binds ~2/3 of decisions and a trajectory readback answers it backwards, and a representation question that resolves as an **interaction with policy class** rather than a property of the domain. Re-contributed whole on 2026-09-24, replacing the first campaign; that one is published at **v0.11.5**. | v0.11.0 | v0.11.5 |
...
| `dynamic_pricing` — finite-horizon dynamic pricing of a fixed stock | Guillermo Gallego and Garrett van Ryzin (1994), "Optimal Dynamic Pricing of Inventories with Stochastic Demand over Finite Horizons", *Management Science* **40**(8), 999–1020. The regular exponential-demand case, discretized into 50 periods. | A **decision-conditioned generator**: Poisson arrivals whose rate depends on the current price while the seed key carries no decision, so realized randomness stays path-independent — a claim `mdp_ir.laws` cannot assert and the domain's own test does. Two declared action encodings of one decision, where the paper's native intensity control is the harder one for PPO by 15 points (the raw-zero dead zone — the source of `mdp-build`'s action-box lesson). An `exact` DP bar with the paper's fluid heuristics beside it, and a second board (`ample_stock`) where the bar and the heuristic coincide. Its RL rows are a July 2026 record on the pre-v2 seed key with no surviving artifact; its benchmark rows were re-measured at v0.10.11 and agree within two SE. Withdrawn 2026-09-14 in the pre-§1.3 shape; returned the same day in the current one. | pre-release (2026-07-09, in-tree `mdp_ir` at IR v0.4) | v0.10.11 |
...
`adi_flex` re-gated at **v0.10.5** (harness byte-identical to v0.10.4) on 2026-09-04 with its re-contribution:
...
head gate G1–G5 and the action-mode probe 10/10 exact (its four solved tables
come from README §4's benchmark commands); every module imports under the
documented install (the case carries its own Poisson helpers — the repository
stays scipy-free). The
...
23 instances, `signal_stdevs` in one). It WARNs rather than FAILs because the
...
`built` tag therefore stayed v0.8.0 while its `gated` tag moved to v0.9.5 (and
has moved with each re-gate since), which is the intended state for a case
whose declarations have been carried forward and whose results have been left
alone. The three checks it turns from SKIP to PASS
...
**`adi_flex` meets the contract below since 2026-09-04** (PR #80's
...
carried no campaign record. `secretary` is now the compact reference for the
shape of a case; `clark_scarf` and `adi_flex` are the full-length ones — read
either for what a campaign record, a §14 readback and a §10 playbook look like
at scale. `clark_scarf` stopped being the compact one with its second campaign
(2026-09-24), which added five modules and a full record.
...
It returned on 2026-09-14, the same day, in the §1.3 shapes: `CLAUDE.md`,
the four-section README, a campaign log seeded from the July record, a
backfilled restatement and a playbook, with its four benchmark arms re-measured
at v0.10.11. Its RL rows stay the July record until its author re-trains them
on the v2 seed tree, and its sign-off and run plan are still owed — both
listed in its `ESCALATION.md` frontier rather than hidden by the return.
