...
> ### ⚠ Scope of every result below: the GAUSSIAN branch only
>
> **The Bernoulli branch was never run.** It has no bar, no baselines and no
> trained policy — `results/bern_K10_T1000/` does not exist. This is the
> campaign's one **accepted coverage debt** (`A4`, ESCALATION §Frontier item
> 7), closed out deliberately rather than paid, and it means the campaign is
> **not complete by its own rules**: the root is a coverage split, so
> finishing Gaussian does not discharge Bernoulli.
>
> Read every claim in this README as scoped to Gaussian payouts. In
> particular these three, which are the ones most likely to be quoted:
>
...
>   Bernoulli is **untested** (#E27 pre-registered A4 as exactly that test);
...
>
> Paying the debt is cheap — 4 benchmark evals, no training
> (`mab_benchmark_{random,greedy,ucb1,thompson}_eval.py -s bern_K10_T1000`).
> It was left open as a scoping decision, not because it is hard.
...
Fingerprints as of 2026-07-29: `mdp` `546617c32435`, structural `072c7f55c967`
...
`UPSTREAM_PROPOSAL_l1_derivation_units.md`.
...
up with at the longest — see `UPSTREAM_PROPOSAL_generalist_lambda.md`.
...
|---|---|---|---|
| `mab_mdp` + `mab_gym` (**canonical**) | *is* the domain | ~1x | `mdp_ir.differential` vs the IR interpreter, base + all 21 instances |
...
python -m mdp_ir.differential cases/mab/mab_schema.json --all-instances --episodes 40
pytest cases/mab/mab_test.py                             # 68 domain tests
pytest mab_test.py                               # needs the [dev] extra (pytest)
...
# the two-constant RULE — the top leaderboard row (1486.14), no network needed
OMP_NUM_THREADS=1 python mab_policy_probe.py --part csweep --n-seeds 8192
# gate
python -m mdp_gates \
