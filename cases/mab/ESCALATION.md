...
option: `✗`/`∅` are illegal on an S-edge, completing T1 does not reduce the
obligation, and the campaign cannot close while S2 is outstanding. **It did not:
A4 was closed out as accepted debt by operator decision 2026-08-11, so this
campaign is recorded closed-WITH-debt, and every T1 claim ships scoped to
Gaussian payouts.** Everything
...
graph TD
    ...
    ROOT -.->|"S2 ⚠ ACCEPTED DEBT — campaign NOT complete"| T2["<b>T2 bernoulli</b> — bern_K10_T1000<br/><b>coverage debt (A4), never paid</b> — no bar, no baselines, no results dir<br/>closed out by operator decision 2026-08-11, not discharged<br/>every T1 claim is therefore scoped to GAUSSIAN payouts<br/>also the untested transfer test for the A9 constant (#E27)"]
...
one (`UPSTREAM_PROPOSAL_l1_derivation_units.md`).
...
6. **A3 — `mdp_tuning` L2(hp) @ T1/bayes/L1/index** ✓ *consumed → #E28* —
   ...
   nearly is.
7. **A4 — open S2: the bar for T2 bernoulli** ⚠ ***ACCEPTED DEBT, never paid
   (operator decision 2026-08-11 at wrap-up)*** — the branch has no bar on the
   current stream, and now none is coming. Cheap (4 evals, no training) and
   the prerequisite for every later T2 item. Not optional and not off-tree:
   completing T1 does not reduce this obligation (guide §3.5), so **the
   campaign closes on A3 + A4, not A3 alone** — and A3 alone is what it got.
   **The campaign is therefore NOT complete by its own rules, and is recorded
   as closed-with-debt rather than closed.** Its transfer value is what is
   forgone: it would have said whether "bayes ≫ stats" (+458.73, the largest
   lever here) — and any A6/A7 win, and #E26's fitted 2.5 — is a fact about
   bandits or a fact about *Gaussian* bandits. Nothing in the campaign
   separates those, so **every T1 claim ships scoped to Gaussian payouts**
   (README carries the same warning at the top).
   Reopening costs minutes:
...
rule:     two, one local and one upstream.
          ...
          **Upstream:** `bounds`/`element_bounds` should accept a constant
          ...
          `UPSTREAM_PROPOSAL_bounds_name_constants.md`.
...
`UPSTREAM_PROPOSAL_tuning_selection_validity.md`.
...
`UPSTREAM_PROPOSAL_checkpoint_selection.md`).
...
`UPSTREAM_PROPOSAL_l1_derivation_units.md` (§8.6's lambda row restated in
...
prescription: `UPSTREAM_PROPOSAL_generalist_lambda.md` — §5.6 should state that
