...
| `PLAYBOOK.md` | the guide-§10 case-close digest |
...
- **Compute sites/venues are cited by alias, never hostname** — venue config
  lives at the repo root (`sites.toml`, gitignored; `sites.example.toml` is
  the schema); a venue change is a confound to record, not a detail. The
  torch version differs by venue here, which every cross-venue comparison
  carries as a stated confound.
