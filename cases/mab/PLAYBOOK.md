...
symptom:      the study's best trial loses to a lower-ranked trial when both
              are re-evaluated at the reporting protocol.
diagnosis:    the tuner scores a different checkpoint (its own last, not the
              selected one) on a different, easier seed block — so its
              objective and the reporting protocol are not the same function.
prescription: protocol — take the HP CENTRE from the study, then RE-RANK the
              top trials at full protocol before adopting. Trial 36 (1458.67
              by the tuner) lost to trial 6 (1443.15 by the tuner) once both
              were scored honestly; trial 6 became the config of record and
              stayed there. SCOPE: any tuner differing from the protocol in
              checkpoint, seed block, or eval depth — most of them, silently.
...
if their difference is deterministic, the streams are coupled. *Why it hides*:
the law the agent sees is still correct, so it survived three weeks and 28
passing tests before being caught. **F1**, cost: voided #E1–#E4 and the entire
baseline table.
