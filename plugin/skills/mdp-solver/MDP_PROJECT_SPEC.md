...
- **The same defect one level down: a vector enumerated instead of declared.** The width rule above governs the *size*; this governs *what is being sized*, and it is not reachable by `model.boundary` at all — that check SKIPs whenever the theory layer is undeclared, which is exactly the state a hand-unrolled IR tends to be in. Three shapes, all WARNed by `mdp_conformance schema.no_enumeration`:
  - a **decorative constant** — declared, referenced nowhere, while the rendering hardcodes the same quantity independently. `adi_flex`'s first revision (2026-07) declared `demand_window: 2` and mentioned it *once in the whole file*, its own declaration, while `due_now`/`due_next` carried the pipeline by hand 11 and 12 times over. Set `demand_window` to 3 and nothing happened: no third slot, no error, no complaint. The constant was decoration, and the IR silently disagreed with itself. The shipped case no longer has either symbol — `cases/adi_flex/ESCALATION.md` F13 ("a constant defined where nothing reads it, twice") records the removal, which is the reproducible form of this example.
  - an **enumerated family** — one vector written as siblings (`due_now`/`due_next`, `lambda_now`/`lambda_next`/`lambda_later`, `pipe1`/`pipe2` — `adi_flex`'s first revision had all three; `cases/adi_flex/ESCALATION.md` F3 records the enumeration and the re-seed its removal forced, and the shipped IR declares one `lambda_seg` vector with a `T_dl + 1` width and one `pipe` with an `L` width). Declare `due` with `length: "demand_window"` and quantify over it. Unrolling deletes the width site, so nothing can sweep the dimension and no width check can see it — the rule above is evaded rather than broken.
  - a **re-baked vector** — a numeric list restated per instance because its *derivation* cannot be declared (`ScenarioConstant.value` is a literal by design). `clark_scarf`'s `h_install` states its own formula in `desc` and then hardcodes the result in **every** instance, each re-computed by hand and checked by nothing. Until a derived-constant mechanism exists (issue #45) this one is sometimes unavoidable — when it is, the `desc` must state the derivation and every instance that must re-bake it. Baking tends to *strand the inputs* too: the constants the formula consumed have nothing left to feed, so they read as decorative unless something else happens to use them (`fnv`'s `t_last` survives only because the gym observes it).
...
**Documentation is not hashed, from IR 0.5.** A `desc` names what a quantity is for and a `note` qualifies a distribution; neither states a fact the interpreter reads, and no trajectory moves when one is rewritten. Hashing them made a prose correction void a Phase-A sign-off, which is not hypothetical: a case re-signed the same day to accommodate a fix, then left a description knowingly wrong because putting it right would have orphaned its artifacts. From `ir_version` 0.5 both keys are excluded from `mdp_fingerprint()`, on the argument that already excludes a `narrowed` citation — a schema that documents itself must not thereby void its own confirmation. The rule is version-gated because a token that moves under everyone at once reads as if every confirmation had been voided: an IR at 0.4 hashes byte-identically to what it always did, and adoption is the author's move. **Adopting is one edit and one move** — bump `ir_version` to `0.5`, record the new token in `{name}.signoff.json` with the date the human re-confirmed it, carry it into the restatement header and §MAP, and log an §IR-CHANGELOG entry saying the statement stopped including its prose. A later version in the same lineage never moves it again, and an empty `desc` is not the same as none, so blanking the text first does not dodge the one-time move. Runs trained before the bump keep the old `ir_mdp_fingerprint` in their args log: that is history, not a mismatch to repair.
...
- **A generalist observes the law in force by reading it from the slot, never from a constant.** A policy trained across a grid of laws (lead time `[p, 1−2p, p]` on `{1, 2, 3}` with `p` swept) must condition on which law it faces, and the declaration is one derived feature — `{"derived": "leadtime_law", "expr": "leadtime.probs", "dim": 3}` — resolved from the *selected* candidate at load, exactly as a symbolic bound is: the pmf under the stochastic candidate, the one-point law under the deterministic one, the marginal under a mixture (sample §5.2, §7). Naming the constant that parameterises one candidate instead (`leadtime_probs`) is the anti-pattern: it keeps a value under every sibling candidate, shows a law that is not in force, and nothing crashes — the gym ends up asserting a narrowing the IR cannot express. Folding at load is what makes it a law: a feature is evaluated at runtime against the state namespace, where the slot's name is the period's realized draw. The latent guard on features is about the **realization** — a hidden draw's name is barred — and a slot's law is its prior, which a policy may know (upstream #77). The finite-support families derive the law; an unbounded one, or a law under a latent, states it in the candidate's `read_api` block.
