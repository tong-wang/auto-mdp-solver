...
def _load_fitc() -> dict:
    """The fitted-c artifact, or a message saying how to get what you wanted.

    `fitc.json` is written only by `--part fitc`, which replays the campaign's
    trained runs — so both `--part score` and `--part csweep` are unusable on a
    fresh checkout. That is fine for a readback instrument, but the shipped
    RULE was originally scored here, which made the leaderboard's headline
    number unreproducible without the campaign's run directories. It is now a
    benchmark policy with no artifact; this points there.
    """
    path = OUTDIR / "fitc.json"
    if not path.is_file():
        raise SystemExit(
            f"missing {path}\n\n"
            f"It is written by `--part fitc`, which needs the campaign's trained\n"
            f"runs on disk — RUNS in mab_interpret.py matches run names exactly,\n"
            f"timestamps included, so a fresh checkout cannot produce it.\n\n"
            f"If what you want is the shipped rule's score, you need none of\n"
            f"this — it is a benchmark policy with no artifact:\n"
            f"    python mab_benchmark_rule_eval.py -s gauss_K10_T1000 --n-seeds 8192\n"
            f"which writes the spec-§9 TSV and reproduces the leaderboard row.")
